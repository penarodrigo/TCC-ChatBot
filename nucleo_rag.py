...
load_dotenv() # Carrega o .env padrão, ou você pode especificar load_dotenv(dotenv_path='.env')
...
class ConfigManager:
    def __init__(self):
        ...
        logger.debug(f"GOOGLE_API_KEY carregada no ConfigManager: {repr(self.google_api_key)}")
        ...
        self.target_document_name: str = os.getenv("TARGET_DOCUMENT", "IDDC.pdf")
...
class RAGSystem:
    ...
    def _initialize_vector_store(self):
        ...
        if FAISS_AVAILABLE:
            if not self.embedding_model:
                self.logger.error("Não é possível inicializar o FAISS sem um modelo de embedding carregado.")
                self.vector_store = None
                return
            try:
                # Obter a dimensão dinamicamente do modelo carregado
                ...
                self.logger.error(f"Falha ao criar índice FAISS: {e}", exc_info=True)
                self.vector_store = None
        else:
    ...
    def get_answer(self, question: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"Erro embedding pergunta: {e}", exc_info=True)
            return "Erro processando pergunta (embedding)."
        ...
        if not ctx_chunks:
            self.logger.warning(f"Nenhum contexto de '{self.config.target_document_name}' para pergunta.")
        else:
            self.logger.debug(f"Contexto LLM (primeiros 200 chars): {ctx_text[:200]}...")
...
"""
        self.logger.info("Enviando prompt para Gemini...")
        self.logger.debug(f"Prompt (500 chars):\n{prompt[:500]}")
        ...
        try:
            ...
            if response and hasattr(response, "parts"):
                answer_text = "".join(p.text for p in response.parts if hasattr(p, 'text'))
                self.logger.info(f"Resposta Gemini: {answer_text[:100]}...")
                return answer_text.strip()
            else:
                self.logger.warning(f"Resposta Gemini inválida ou vazia: {response}")
                return None
        except google.api_core.exceptions.InvalidArgument as e:
            if "API key not valid" in str(e):
                self.logger.error(f"Erro de autenticação com a API Gemini: Chave de API inválida. Verifique o arquivo .env.", exc_info=True)
                return "Erro de autenticação: A chave da API do Google (GOOGLE_API_KEY) é inválida. Por favor, verifique sua configuração no arquivo .env."
        except Exception as e:
            self.logger.error(f"Erro API Gemini: {e}", exc_info=True)
            return f"Erro comunicação LLM: {str(e)}"
...
def initialize_global_components():
    ...
    try:
        ...
        if current_config.google_api_key and current_config.google_api_key != "your_google_api_key_here":
            ...
            logger.error("GOOGLE_API_KEY não configurada. Gemini indisponível.")
...
def rota_ask_question_api():
    ...
    try:
        ...
        if resposta is None:
            logger.error("A resposta gerada é None. Verifique o modelo ou a chave de API.")
            resposta = "Erro: Não foi possível gerar uma resposta."
